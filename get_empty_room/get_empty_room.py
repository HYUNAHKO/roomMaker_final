...
#TIP: COLOR_MAPPING_에 포함되지 않는 색상은 제외하고 처리하기 위해서 COLOR_MAPPING_RGB을 포함시킴.
from segmentation_colors import ade_palette, map_colors_rgb, COLOR_MAPPING_RGB
...
def get_inpating_mask(segmentation_mask: np.ndarray) -> Image:
    ...
    #TIP: COLOR_MAPPING_RGB에 포함되지 않는 색상은 처리하지 않음.
    unique_colors = [tuple(color) for color in unique_colors if tuple(color) in COLOR_MAPPING_RGB]
...
if __name__ == '__main__':
    ...
    image_paths = f"{home}/roomMaker/dataset/airbnb"
    save_dir = f"{home}/roomMaker/clean_images"
    ...
    for elem in tqdm(all_images):
        ...
        color_map, segmentation_map = get_segmentation_of_room(image) # color_seg = color_map, seg_image = segmentation_map
